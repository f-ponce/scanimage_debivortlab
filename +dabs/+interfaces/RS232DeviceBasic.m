classdef RS232DeviceBasic < most.DClass
    ...
    methods
        ...
        function val = get.beforeEverySerialCommandProps(obj)
            val = struct();
            val.terminator = obj.defaultTerminator;
            val.timeout = obj.defaultTimeout;
        end
    ...
    methods (Hidden)
        ...
        function sendCommandRaw(obj,command)
            if obj.skipTerminatorOnSend || isempty(obj.hSerial.Terminator)
                ...
                if ischar(command)
                    fprintf(obj.hSerial,command); % does not send terminator
                else % numeric vector
                    precision = class(command);
                    fwrite(obj.hSerial,command,precision);
                    fprintf(obj.hSerial,'');
            ...
            if obj.asyncReplyPending
                switch mode
                    case 'error'
                        ...
                        throwAsCaller(MException([most.util.className(obj) ':InvalidCommandDuringPendingAsyncResponse'], 'Command issued cannot proceed while an asynchronous response from device remains pending.'));
                    case 'warning'
                        most.idioms.dispError('WARNING(%s): An asynchronous reply remained pending, but has been aborted. Previously received data is lost\n',mfilename('class'));
                end                
                obj.asyncReplyCleanup();
                obj.flushInputBuffer();
            end
    ...
    methods
        ...
        function data = flushInputBuffer(obj)
            data = [];
            if obj.hSerial.BytesAvailable > 0
                data = obj.readBinaryRaw(obj.hSerial.BytesAvailable);
            end
        end
