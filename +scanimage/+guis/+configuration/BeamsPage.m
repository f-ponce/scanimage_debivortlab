classdef BeamsPage < scanimage.guis.configuration.ConfigurationPage
    ...
    methods
        function obj = BeamsPage(hConfigEditor, create)
            ...
            obj.routingStuff(end+1) = obj.hLineClockInText;
            ...
            obj.routingStuff(end+1) = obj.hLineClockInPopUp.hCtl;
            ...
            obj.routingStuff(end+1) = obj.hFrameClockInText;
            ...
            obj.routingStuff(end+1) = obj.hFrameClockInPopUp.hCtl;
            ...
            obj.routingStuff(end+1) = obj.hReferenceClockInText;
            ...
            obj.routingStuff(end+1) = obj.hReferenceClockInPopUp.hCtl;
        ...
        function refreshPageDependentOptions(obj)
            % Initialize Shutter Page Names
            shutters = obj.hConfigEditor.shutterNames;
            shutters = [repmat({false},numel(shutters),1) shutters];

            curDaq = obj.currDaqSel;
            if ~isnan(curDaq)
                shutterOpenIds = obj.daqSettingList(curDaq).calOpenShutterIDs;
                if ~isempty(shutterOpenIds)
                    shutters(shutterOpenIds) = {true};
                end
            end
        ...
        function updateDaqLists(obj)
            if ~isempty(obj.hConfigEditor.availableDaqs)
                obj.hAvailableDaqDevicesList.String = setdiff(obj.hConfigEditor.availableDaqs(ismember({obj.hConfigEditor.daqInfo.productCategory},{'DAQmx_Val_XSeriesDAQ' 'vDAQ'}))',obj.beamDaqs);
    ...
    methods
        function set.currDaqSel(obj,v)
            ...
            if isempty(v) || isnan(v)
                ...
                isVdaq = ismember(dn,obj.hConfigEditor.availableRios) && dabs.vidrio.rdi.Device.isRdiDeviceName(dn);
                tf = tf || isVdaq;
                ...
                if tf
                    ...
                    % channel table
                    obj.hChannelTable.ColumnFormat = {aos, 'char', 'numeric', [{'None'} ais], 'numeric', {'Transmitted Light' 'Rejected Light'} 'char'};
                    ...
                    % routing section
                    if isVdaq
                        set(obj.routingStuff,'Visible','off')
                        d = 160;
                        
                        obj.hBeamDaqDevicePanel.Position([2 4]) = [32+d 535-d];
                        obj.hChtxt.Position(2) = 463-d;
                        obj.hChannelTable.Position(2) = 344-d;
                        obj.hShutterTableText.Position(2) = 295-d;
                        obj.hShutterTable.Position(2) = 193-d;
                    else
                        set(obj.routingStuff,'Visible','on')
                        obj.hBeamDaqDevicePanel.Position([2 4]) = [32 535];
                        
                        obj.hChtxt.Position(2) = 463;
                        obj.hChannelTable.Position(2) = 344;
                        obj.hShutterTableText.Position(2) = 295;
                        obj.hShutterTable.Position(2) = 193;
                    ...
                    obj.hBeamDaqDevicePanel.Title = sprintf('Beam DAQ %d: %s',v,dn);
                    disablePanel();
                    obj.hNoSelText.String = 'Invalid DAQ Selection!';
                end
            ...
            function disablePanel
                ...
                obj.hNoSelText.String = 'Select a beam DAQ above in order to configure and add channels to it...';
                obj.hShutterTable.Data(:,1) = {false};
                obj.hChannelTable.Data = {};
        ...
        function v = get.beams(obj)
            n = numel(obj.beamDaqs);
            if n
                v = arrayfun(@(s){s.displayNames},obj.daqSettingList);
                v = horzcat(v{:})';
                
                dn = arrayfun(@(s){repmat(s,1,numel(obj.daqSettingList(s).displayNames))},1:numel(obj.daqSettingList));
                v(:,2) = num2cell(horzcat(dn{:})');
                
                v(:,1) = cellfun(@(n,id){sprintf('%s (%s)',n,obj.beamDaqs{id})},v(:,1),v(:,2));
            else
                v = {};
            end
        end
