    ...
    methods
        function obj = IntegrationROIOutputsPage(hConfigEditor, create)
            ...
            if ~isempty(obj.hConfigEditor.availableDaqs)
                formatDaqs = cat(2, 'None', obj.hConfigEditor.availableDaqs');
                formatChannels = cat(2, 'None', cat(2, obj.getAnalogOutputChannelList(), obj.hConfigEditor.allOutputDigChans'));
            else
                formatDaqs = 'char';
                formatChannels = 'char';
            end
        ...
        function reload(obj)            
            ...
            if isempty(mdfData) || ~numel(mdfData.deviceNames) || ~numel(mdfData.deviceChannels)
                ...
                emptyCells = cellfun(@(s)isempty(s) || strcmpi(s,'none'),mdfData.deviceNames);       
                mdfData.deviceNames(emptyCells) = {'None'};               

                emptyCells = cellfun(@(s)isempty(s) || strcmpi(s,'none'),mdfData.deviceChannels);       
        ...
        function aoList = getAnalogOutputChannelList(obj)
            ...
            if ~isempty(obj.hConfigEditor.availableDaqs)
                ...
                aoList = arrayfun(@(i){sprintf('AO%02d',i)},0:N-1);
            end
        end
    end
