    ...
    methods
        ...
        function refreshPageDependentOptions(obj, mdfData)
            ...
            if ~isempty(scannerNames)
                scannerNames(~obj.hConfigEditor.scannerCanPhotostim) = [];
            end

            if ~isempty(scannerNames)
                obj.hPhotoStimulationScannerPopUp.String = [{'None'}; scannerNames];
            else
        ...
        function aiList = getAnalogInputChannelList(obj, daqName)
            ...
            if tf
                aiList = [{'None'} obj.hConfigEditor.daqInfo(idx).allAIs];
            else
