            ...
            try
                ...
                if evalin('base','exist(''hSI'')')
                    ...
                    if most.idioms.isValidObj(hSI)
                        nm = sprintf('vDAQ%d',daqId);
                        obj.ownFpga = ~hSI.fpgaMap.isKey(nm);
                        if ~obj.ownFpga
                            s = hSI.fpgaMap(nm);
                            obj.hFpga = s.hFpga;
                            obj.hDelListener = most.ErrorHandler.addCatchingListener(obj.hFpga,'ObjectBeingDestroyed',@(varargin)obj.delete);
                        end
                    end
                end
                
                if obj.ownFpga
                    if obj.deviceInfo.numClients > 0
                    ...
                    obj.hFpga = scanimage.fpga.vDAQ_SI(obj.vdaqId,obj.deviceInfo.simulated);
                end
                obj.isR1 = obj.hFpga.isR1;
                obj.numAo = 5 + 7*obj.isR1;
                obj.numAi = 4 + 8*obj.isR1;
                ...
                most.gui.uicontrol('parent',rowflow,'style','popupmenu','string',{'2 Vpp' '1 Vpp' '0.5 Vpp'},textProps{:},'Bindings', {obj 'hsaiInputRangeSel' 'value'});
                
                rowflow = most.gui.uiflowcontainer('parent',hsaiOptionFlowT,'flowdirection','lefttoright','HeightLimits',30);
                most.gui.staticText('parent',rowflow,'String','Filter Bandwidth:','horizontalalignment','right','WidthLimits',130,textProps{:});
                most.gui.uicontrol('parent',rowflow,'style','edit','Bindings',{obj 'hsaiFilter' 'string'},textProps{:});
                
                rowflow = most.gui.uiflowcontainer('parent',hsaiOptionFlowT,'flowdirection','lefttoright','HeightLimits',30);
                most.gui.staticText('parent',rowflow,'String','Sample Rate (MHz):','horizontalalignment','right','WidthLimits',130,textProps{:});
                most.gui.uicontrol('parent',rowflow,'style','edit','Bindings',{obj 'hsaiSampleRate' 'value' '%.2f' 'scaling' 1e-6},textProps{:});
                
                rowflow = most.gui.uiflowcontainer('parent',hsaiOptionFlowT,'flowdirection','lefttoright','HeightLimits',30);
                most.gui.staticText('parent',rowflow,'String','Time Span (ms):','horizontalalignment','right','WidthLimits',130,textProps{:});
                most.gui.uicontrol('parent',rowflow,'style','edit','Bindings',{obj 'hsaiTimeSpan' 'value' '%.2f' 'scaling' 1e3},textProps{:});
                
                rowflow = most.gui.uiflowcontainer('parent',hsaiOptionFlowT,'flowdirection','lefttoright','HeightLimits',30);
                most.gui.staticText('parent',rowflow,'String','Autoscale Axes:','horizontalalignment','right','WidthLimits',130,textProps{:});
                most.gui.uicontrol('parent',rowflow,'style','checkbox',textProps{:},'Bindings',{obj 'hsaiAutoScale' 'value'});
                ...
                for i = 0:3
                    channelPannel = most.gui.uipanel('parent',hsaiFlow,'bordertype','none');
                    obj.hHsaiAx(end+1) = axes('parent',channelPannel,'box','on','xticklabel',[],'xgrid','on','ygrid','on','ylim',2^13*[-1.1 1.1]);
                ...
                obj.hLsaiAx = axes('parent',plotPannel,'title',sprintf('AI%d',i),'box','on','xticklabel',[],'xgrid','on','ygrid','on','ylim',10.1*[-1 1]);
                for i = 0:(3 + 8*obj.isR1)
                    ...
                    a = axes('parent',p,'title',sprintf('AI%d',i),'box','on','xticklabel',[],'xlim',[0 1],'ylim',[0 1],'xcolor','none','ycolor','none','color','none');
                ...
                %% load fpga
                if obj.ownFpga
                    if  ~obj.deviceInfo.simulated
                        h.Children.Children.String = 'Initializing FPGA...';
                        drawnow();
                        obj.hFpga.loadDesign();
                    end
                    
                    try
                        obj.hFpga.configureInternalSampleClock();
                        obj.hsaiInputRangeSel = 1;
                        showWarning = false;
                    catch
                        showWarning = true;
                    end
                else
                    % get input range setting from si
                    sel = 1;
                    if ~isempty(obj.hFpga.oldInputRs)
                        [tf,idx] = ismember(obj.hFpga.oldInputRs(1),[2 1 .5]);
                        if tf
                            sel = idx;
                        end
                    end
                ...
                obj.hAcqEngine = obj.hFpga.hAcqEngine(1);
                obj.hsaiFilter = '30 MHz';
...
%                 st = tic;
                ...
                try
                    if isempty(obj.lastTempUpdate) || (toc(obj.lastTempUpdate) > 1)
                        ...
                        tStr = sprintf('%.1f%cC',obj.hFpga.hSysmon.temperature,char(176));
                    ...
                    if obj.hsaiActive
                        if obj.hAcqEngine.scopeStatusFifoOverflowCount
                            [rateBPS, bytes, ticks] = obj.hFpga.fifo_DataScopeTargetToHost(1).calcDataRate();
                        ...
                        elseif obj.hFpga.fifo_DataScopeTargetToHost(1).getUnreadCount() >= obj.hsaiDataSize
                            
                            dat = obj.hFpga.fifo_DataScopeTargetToHost(1).read(obj.hsaiDataSize, 'int16');
                            N = size(dat,1)/5;
                            
                            xdat = (1:N)';
                            set(obj.hHsaiLine,'XData',xdat);
                            set(obj.hHsaiAx,'XLim',[1 N]);
                            for i = 1:4
                                obj.hHsaiLine(i).YData = dat(i:5:end)/4;
                            end
...
%                 fprintf('Duration: %.4fs\n',toc(st))
        ...
        function startHsaiMonitoring(obj,varargin)
            ...
            N = round(obj.hsaiTimeSpan*obj.hsaiSampleRate);
            obj.hsaiDataSize = N * 10;
            
            if ~obj.hFpga.fifo_DataScopeTargetToHost(1).configured || (obj.hFpga.fifo_DataScopeTargetToHost(1).hostBufferSize < obj.hsaiDataSize)
                obj.hFpga.fifo_DataScopeTargetToHost(1).configure(obj.hsaiDataSize);
            else
                obj.hFpga.fifo_DataScopeTargetToHost(1).flush();
            end
            
            obj.hAcqEngine.resetDataScope();
            obj.hAcqEngine.scopeParamDecimationLB2 = floor(log2(obj.hFpga.sampleClkRate/obj.hsaiSampleRate));
            obj.hAcqEngine.scopeParamNumberOfSamples = N;
            obj.hAcqEngine.scopeParamTriggerHoldoff = 0;
    ...
    methods
        ...
        function set.hsaiSampleRate(obj,v)
            baseRate = obj.hFpga.sampleClkRate*1e6;
        ...
        function set.hsaiFilter(obj,v)
            if isempty(v) || (ischar(v) && ismember(lower(v),{'none' 'bypass' 'fbw'})) || (isnumeric(v) && ~v)
                obj.hFpga.setChannelsFilter(nan);
                obj.hsaiFilter = 'Bypass';
            else
                if ischar(v)
        ...
        function set.hsaiAutoScale(obj,v)
            ...
            if ~v
                set(obj.hHsaiAx, 'YLim', 1.1 * 2^13 * [-1 1]);
            else
                obj.hsaiScaleHistory = {[] [] [] []};
                obj.autoscaleHsai();
            end
        ...
        function set.hsaiInputRangeSel(obj,v)
            ...
            rgs = {[-1 1] [-.5 .5] [-.25 .25]};
            obj.hFpga.setChannelsInputRanges(rgs(v));
        end
