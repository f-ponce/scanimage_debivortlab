classdef RoiGroup < scanimage.mroi.RoiTree    
    ...
    methods        
        % public
        function [ao_volts,samplesPerTrigger,sliceScanTime,path_FOV] = scanStackAO(obj,scannerset,zPowerReference,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ)
            if nargin < 8 || isempty(zActuator)
                zActuator = 'fast';
            end
            if nargin < 9 || isempty(sliceScanTime)
                sliceScanTime = [];
            end
            if nargin < 10 || isempty(tuneZ)
                tuneZ = true;
            end
            
            [path_FOV,samplesPerTrigger,sliceScanTime] = obj.scanStackFOV(scannerset,zPowerReference,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ);
            ao_volts = arrayfun(@(fov)scannerset.pathFovToAo(fov),path_FOV);
        end
        
        % private
        function [path_FOV,samplesPerTrigger,sliceScanTime] = scanStackFOV(obj,scannerset,zPowerReference,zs,zsRelative,zWaveform,flybackFrames,zActuator,sliceScanTime,tuneZ,maxPtsPerSf,applyConstraints)
            if nargin < 8 || isempty(zActuator)
                zActuator = 'fast';
            end
            if nargin < 9
            ...
            if nargin < 11 || isempty(maxPtsPerSf)
                maxPtsPerSf = inf;
            end
            if nargin < 12 || isempty(applyConstraints)
            ...
            for idx = numel(zs) : -1 : 1
                [outputData{idx}, slcEmpty(idx)] = obj.scanSliceFOV(scannerset,zPowerReference,zs(idx),zsRelative(idx),dzdt,zActuator,frameTime,flybackTime,zWaveform,maxPtsPerSf);
            end
        ...
        function [path_FOV, slcEmpty] = scanSliceFOV(obj,scannerset,zPowerReference,z,zRelative,dzdt,zActuator,frameTime,flybackTime,zWaveformType,maxPtsPerSf)
            ...
            if numel(scanRois) > 0
                ...
                for i = 1:numel(scanRois)
                    [paths{end+1}, dt] = scanRois(i).scanPathFOV(scannerset,zPowerReference,z,actz,zRelative,actzRelative,dzdt,zActuator,tfStim,maxPtsPerSf);
    ...
    methods
        ...
        function tf = isequalish(obj,other)
            tf = false;
            
            if ~isempty(obj.rois) && numel(obj.rois)==numel(other.rois)
                tfmask = arrayfun(@(a,b)a.isequalish(b),obj.rois,other.rois);
                tf = all(tfmask);
            end
        end
