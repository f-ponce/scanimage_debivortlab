classdef GalvoGalvo < scanimage.mroi.scannerset.ScannerSet
    ...
    methods
        ...
        function ao_volts = pathFovToAo(obj,path_FOV)
            ...
            if obj.hasSlm && isfield(path_FOV, 'SLM')
                ...
                if obj.hasBeams
                    path_FOV.B = path_FOV.B / ao_volts.SLM.mask.efficiency;
                    path_FOV.B = min(path_FOV.B,1);
                end
            end
            
            if obj.hasBeams
                bIDs = obj.beams.beamIDs;
                ao_volts.B = zeros(size(path_FOV.B),'like',path_FOV.B);
                for i = 1:numel(bIDs)
        ...
        function [path_FOV, seconds] = scanPathFOV(obj,scanfield,roi,zPowerReference,actz,actzRelative,dzdt,zActuator,maxPtsPerSf)
            ...
            if isa(scanfield,'scanimage.mroi.scanfield.ImagingField')
                [path_FOV, seconds] = obj.scanPathImagingFOV(scanfield,roi,zPowerReference,actz,actzRelative,dzdt,zActuator);
            elseif isa(scanfield,'scanimage.mroi.scanfield.fields.StimulusField')
                [path_FOV, seconds] = obj.scanPathStimulusFOV(scanfield,zPowerReference,actz,actzRelative,dzdt,[],[],maxPtsPerSf);
            else
    ...
    methods(Hidden)
        ...
        function [path_FOV, seconds] = scanPathImagingFOV(obj,scanfield,roi,zPowerReference,actz,actzRelative,dzdt,zActuator)
...
%                 end
                ...
                path_FOV.Z = obj.fastz.scanPathFOV(obj,zPowerReference,actz,actzRelative,dzdt,seconds,path_FOV.G);
            end
            ...
            function path_FOV = generateBeamsPathImaging(path_FOV)               
                ...
                if isempty(obj.acqSampleRate)
                    ...
                    nx = obj.nsamples(obj.beams,lineScanPeriod);           % total number of scan samples per line
                    nBlank = (nx - nxAcq) / 2;
                    assert(rem(nBlank,1)==0); % sanity check: at the moment we only support even number of samples per line
                else
                    ctlFs = obj.beams.sampleRateHz;
                    nx = ctlFs * lineScanPeriod;
                    assert(nx == round(nx), 'Invalid sample rate');
...
%                 powerFracsPb(isnan(powerFracsPb)) = 0;
        ...
        function [path_FOV, seconds] = scanPathStimulusFOV(obj,scanfield,zPowerReference,actz,actzRelative,dzdt,transform,scanBeams,maxPoints)
            if nargin < 7 || isempty(transform)
                transform = true;
            end
            
            if nargin < 8 || isempty(scanBeams)
                scanBeams = true;
            end
            
            if nargin < 9 || isempty(maxPoints)
                maxPoints = inf;
            end
            ...
            if obj.hasFastZ
                ...
                elseif isfield(path_FOV,'Z')
                    startz = path_FOV.Z+actzRelative;
                    endz = [];
                else
                    startz = actzRelative;
                    endz = actzRelative;
