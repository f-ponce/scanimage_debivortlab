classdef ScannerSet < handle
    ...
    methods
        ...
        function [t,wDes,wCmd] = zWvfm(obj,roiGroup,zPowerReference,zs,zsRelative,fb,wvType)
            if obj.hasFastZ
                % generate ao using scannerset
                [~,~,~,fovd] = roiGroup.scanStackAO(obj,zPowerReference,zs,zsRelative,wvType,fb,[],[],false);
                [~,~,~,fovc] = roiGroup.scanStackAO(obj,zPowerReference,zs,zsRelative,wvType,fb,[],[],true);
                wDes = fovd.Z;
                wCmd = fovc.Z;
