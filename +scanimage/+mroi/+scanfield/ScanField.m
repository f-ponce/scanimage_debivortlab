classdef ScanField < scanimage.mroi.RoiTree
    ...
    methods(Abstract)        
        ...
        % returns true if a 2D point (X,Y), is within the scanfield
        tf = isPtInScanField(obj,pt_2D);
    ...
    methods(Access = protected)
        ...
        function fireChangedEvent(obj,propName,oldVal,newVal)
            if nargin > 1
            ...
            if changed
                fireChangedEvent@scanimage.mroi.RoiTree(obj,scanimage.mroi.EventData(obj,'property',propName,oldVal,newVal));
            end
        end
    end
    ...
    methods        
        function set.centerXY(obj,val)
            ...
            obj.fireChangedEvent('centerXY',oldVal,val);
        end
        ...
        function set.sizeXY(obj,val)
            ...
            obj.transformParams_ = [];
            obj.fireChangedEvent('sizeXY',oldVal,val);
        end
        ...
        function set.rotationDegrees(obj,val)
            ...
            obj.transformParams_ = [];
            obj.fireChangedEvent('rotationDegrees',oldVal,val);
        end
        
        function set.enable(obj,val)
            oldVal = obj.enable;
            obj.enable = logical(val);
            obj.fireChangedEvent('enable',oldVal,val);
        end
