classdef RoiData < handle & matlab.mixin.Copyable
    ...
    methods
        ...
        function mergeIn(obj,newRoiData)
            [tf, srcIdx] = ismember(obj.channels,newRoiData.channels);
            assert(all(tf), 'Requested channels not available.');
            
            obj.stripePosition = cellfun(@(oldpos,newpos){[oldpos(1) newpos(2)]}, obj.stripePosition, newRoiData.stripePosition);
            
            for iterChannels = 1:length(obj.imageData)
                for iterZs = 1:length(obj.imageData{iterChannels})
                    pos = newRoiData.stripePosition{iterZs};
                    posns = pos(1):pos(2);
                    if obj.transposed
                        obj.imageData{iterChannels}{iterZs}(:,posns) = cast(newRoiData.imageData{srcIdx(iterChannels)}{iterZs}(:,posns),'like',obj.imageData{iterChannels}{iterZs});
                    else
                        obj.imageData{iterChannels}{iterZs}(posns,:) = cast(newRoiData.imageData{srcIdx(iterChannels)}{iterZs}(posns,:),'like',obj.imageData{iterChannels}{iterZs});
                    end
                end
