classdef Control < scanimage.interfaces.Class
    ...
    properties (SetAccess = private)
        ...
        hAO_Manual;                             % handle to either hAO_Manualxyb or hAO_Manualxybz depending on if fastz is active
        hAO_Manualxyb;                          % handle of Manual AO Task for reposition of an analog-controlled X/Y scanner pair
        hAO_Manualxybz;                         % handle of Manual AO Task for reposition of an analog-controlled X/Y scanner pair and fastz actuator
    ...
    methods
        ...
        function delete(obj)
            most.idioms.safeDeleteObj(obj.hAO_Manualxyb);
            most.idioms.safeDeleteObj(obj.hAO_Manualxybz);
    ...
    methods
        function start(obj)
            ...
            obj.hLinScan.xGalvo.hDevice.pointPositionVoltage(obj.hLinScan.hSI.hWaveformManager.scannerAO.ao_volts.G(end, 1));
            obj.hLinScan.yGalvo.hDevice.pointPositionVoltage(obj.hLinScan.hSI.hWaveformManager.scannerAO.ao_volts.G(end, 2));
...
%                     warning('Output buffer is empty. Cannot restart. Starting a new generation instead');
        ...
        function abort(obj) 
            try
                ...
                if ~isempty(obj.lastKnownAOBuffer)
                    obj.hLinScan.xGalvo.hDevice.pointPositionVoltage(obj.lastKnownAOBuffer(1,1));
                    obj.hLinScan.yGalvo.hDevice.pointPositionVoltage(obj.lastKnownAOBuffer(1,2));
                end
                ...
                if isfield(obj.hLinScan.hSI.hWaveformManager.scannerAO.pathFOV, 'B')
                    obj.updateBeamStatusFrac(obj.hLinScan.hSI.hWaveformManager.scannerAO.pathFOV.B(end, 1));
                end
        ...
        function updateLastKnownGalvoPositionFromOutputTask(obj)
            ...
            if ~isempty(numSamplesGenerated)
                ...
                if idx > 0
                    obj.hLinScan.xGalvo.hDevice.lastKnownPositionVoltage = obj.lastKnownAOBuffer(idx,1);
                    obj.hLinScan.yGalvo.hDevice.lastKnownPositionVoltage = obj.lastKnownAOBuffer(idx,2);
                end
            end
        end
        ...
        function updateAnalogBufferAsync(obj,restartTask)
            ...
            if framesToWrite > 0
                if restartTask
                    ...
                    obj.hLinScan.xGalvo.hDevice.pointPositionVoltage(newBuffer(1,1));
                    obj.hLinScan.yGalvo.hDevice.pointPositionVoltage(newBuffer(1,2));
    ...
    methods (Access = private)
        ...
        function ziniPrepareTasks(obj)
            ...
            tName = [obj.hLinScan.name '-ScannerOutManual'];
            obj.hAO_Manualxyb = dabs.ni.rio.fpgaDaq.fpgaDaqAOTask.createTaskObj(tName, obj.hFpgaDaq);
            obj.hAO_Manualxyb.createAOVoltageChan(linScanMDF.deviceNameGalvo, linScanMDF.XMirrorChannelID, 'XMirrorChannel');
            obj.hAO_Manualxyb.createAOVoltageChan(linScanMDF.deviceNameGalvo, linScanMDF.YMirrorChannelID, 'YMirrorChannel');
            ...
            if obj.beamShareGalvoDAQ
                for i = 1:obj.hBeams.daqNumBeams(linScanMDF.beamDaqID)
                    ...
                    obj.hAO_Manualxyb.createAOVoltageChan(linScanMDF.deviceNameGalvo,beamMDF.beamDaqs(linScanMDF.beamDaqID).chanIDs(i),obj.hBeams.displayNames{obj.hBeams.daqBeamIDs{linScanMDF.beamDaqID}(i)});
                end
            end
            ...
            if obj.zShareGalvoDAQ
                ...
                obj.hAO_Manualxybz = dabs.ni.rio.fpgaDaq.fpgaDaqAOTask.createTaskObj([tName 'WZ'], obj.hFpgaDaq);
                obj.hAO_Manualxybz.createAOVoltageChan(linScanMDF.deviceNameGalvo, linScanMDF.XMirrorChannelID, 'XMirrorChannel');
                obj.hAO_Manualxybz.createAOVoltageChan(linScanMDF.deviceNameGalvo, linScanMDF.YMirrorChannelID, 'YMirrorChannel');
                ...
                if obj.beamShareGalvoDAQ
                    for i = 1:obj.hBeams.daqNumBeams(linScanMDF.beamDaqID)
                        ...
                        obj.hAO_Manualxybz.createAOVoltageChan(linScanMDF.deviceNameGalvo,beamMDF.beamDaqs(linScanMDF.beamDaqID).chanIDs(i),obj.hBeams.displayNames{obj.hBeams.daqBeamIDs{linScanMDF.beamDaqID}(i)});
                    end
                end

                % channel for fastz
                obj.hAOxybz.createAOVoltageChan(zMdf.daqDeviceName, zMdf.cmdOutputChanID, 'FastZChannel');
                obj.hAO_Manualxybz.createAOVoltageChan(zMdf.daqDeviceName, zMdf.cmdOutputChanID, 'FastZChannel');
            end
            ...
            obj.hAO = obj.hAOxyb;
            obj.hAO_Manual = obj.hAO_Manualxyb;
