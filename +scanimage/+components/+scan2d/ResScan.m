classdef ResScan < scanimage.components.Scan2D & most.HasMachineDataFile
    ...
    %% Lifecycle
    methods
        function obj = ResScan(hSI, name, legacymode)
            % ResScan constructor for scanner object
            %  obj = ResScan(hSI)
            %  obj = ResScan(hSI, name)
            %  obj = ResScan(hSI, name, legacymode)
            
            if nargin < 2 || isempty(name)
                name = 'Resonant';
            end
            
            if nargin > 2 && ~isempty(legacymode) && legacymode
            ...
            obj = obj@scanimage.components.Scan2D(hSI,name,legacymode);
            ...
            validateattributes(obj.mdfData.polygonalScanner,{'logical','numeric'},{'scalar','binary'});
            if obj.mdfData.polygonalScanner
            ...
            dev = obj.mdfData.rioDeviceID;
            if obj.hSI.fpgaDataScopeMap.isKey(dev)
                obj.hDataScope = obj.hSI.fpgaDataScopeMap(dev);
            else
                obj.hDataScope = scanimage.components.scan2d.resscan.FpgaDataScope(obj);
                obj.hSI.fpgaDataScopeMap(dev) = obj.hDataScope;
            end
        end
    ...
    methods
        ...
        function set.uniformSampling(obj,v)
            if obj.componentUpdateProperty('uniformSampling',v)
                if v ~= obj.uniformSampling
                    if obj.mdfData.polygonalScanner && ~v
                        obj.uniformSampling = true;
        ...
        function val = get.beamDaqID(obj)
            val = obj.mdfData.beamDaqID;
        end
        
        function val = get.beamIds(obj)
            val = [];
            if ~isempty(obj.beamDaqID)
                val = obj.hSI.hBeams.daqBeamIDs{obj.beamDaqID};
            end
        end
    end
...
%             obj.hAcq.flagResizeAcquisition = true;
    ...
    methods (Hidden)
        ...
        function pixPerLine = getPixPerLine(obj)
            if obj.uniformSampling
                linesPerPeriod = 2^(~obj.mdfData.polygonalScanner);
        ...
        function loadClassData(obj)
            ...
            if isempty(obj.mdfData.extendedRggFov)
                try
                    obj.mdfData.extendedRggFov = ~obj.getClassDataVar('resonantLimitedFovMode',obj.classDataFileName);
                catch
                    obj.mdfData.extendedRggFov = ~isempty(obj.mdfData.galvoAOChanIDX) && (obj.mdfData.xGalvoAngularRange > obj.mdfData.resonantAngularRange);
                end
            end
...
%                 end
        ...
        function newPhase = calibrateLinePhase(obj)
            assert(~obj.mdfData.polygonalScanner,'Automatic line phase calibration is unsupported when using a polygonal scanner');
...
function s = defaultMdfSection()
    s = [...
        ...
        makeEntry('polygonalScanner',false,'(optional) Set to true if using a polygonal scanner instead of a resonant scanner')...
        ...
        makeEntry('customSigCondOption','','String: Alternate signal conditioning option')...
        ...
        makeEntry('extendedRggFov',[],'If true and x galvo is present, addressable FOV is combination of resonant FOV and x galvo FOV.')...
        makeEntry()... % blank line
        ...
        makeEntry()... % blank line
        makeEntry('resonantAngularRange',26,'max range in optical degrees (pk-pk) for resonant')...
        makeEntry('rScanVoltsPerOpticalDegree',0.19230769,'resonant scanner conversion factor from optical degrees to volts')...
        makeEntry()... % blank line
