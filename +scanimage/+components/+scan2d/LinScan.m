classdef LinScan < scanimage.components.Scan2D & most.HasMachineDataFile    
    ...
    properties (SetAccess = immutable)
        scannerType = 'GG';
        hasXGalvo = true;                   % logical, indicates if scanner has a resonant mirror
    ...
    %% Lifecycle
    methods
        function obj = LinScan(hSI, name, legacymode)
            ...
            if ~isempty(obj.deviceNameGalvoFeedback)
            ...
            dev = obj.mdfData.deviceNameAcq;
            if obj.hSI.fpgaDataScopeMap.isKey(dev)
                obj.hDataScope = obj.hSI.fpgaDataScopeMap(dev);
            else
                hDS = scanimage.components.scan2d.resscan.FpgaDataScope(obj);
                if most.idioms.isValidObj(hDS)
                    obj.hDataScope = hDS;
    ...
    methods
        ...
        function [fsOut,xWvfm,cmdWvfm,fsIn,respWvfm,lineScanPeriod,lineAcquisitionPeriod] = waveformTest(obj)
            ...
            [ao_volts_optimized,~,~] = obj.hSI.hRoiManager.currentRoiGroup.scanStackAO(ss,zPowerReference,zs(1),zsRelative(1),'',0,[]);
            [ao_volts,~,~] = obj.hSI.hRoiManager.currentRoiGroup.scanStackAO(ss,zPowerReference,zs(1),zsRelative(1),'',0,[],[],[],false);
    ...
    methods (Hidden)
        ...
        function arm(obj)
            ...
            if ~isempty(obj.hSlmScan)
                hCS = obj.hSlmScan.hCSSlmOffset.hParent();
    ...
    methods
        ...
        function val = get.controllingFastZ(obj)
            val = strcmp(obj.mdfData.deviceNameGalvo,obj.hSI.hFastZ.hScanner.positionDeviceName);
        end
...
function s = defaultMdfSection()
    s = [...
        ...
        makeEntry()... % blank line
        makeEntry('fpgaModuleType','','String: Type of FlexRIO FPGA module in use when deviceNameAcq is an FPGA. One of {''NI7961'' ''NI7975''}')...
        makeEntry('digitizerModuleType','','String: Type of digitizer adapter module in use when deviceNameAcq is an FPGA. One of {''NI5732'' ''NI5734''}')...
        makeEntry('customSigCondOption','','String: Alternate signal conditioning option when deviceNameAcq is an FPGA')...
