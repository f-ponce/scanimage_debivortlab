        ...
        function reprocessEstimatorsAndCorrector(obj)
            ...
            for idx = 1:length(hMEsOld)
                try
                    ...
                    most.ErrorHandler.logAndReportError(MatlabException);
                end
            end
...
%                 obj.setClassDataVar('lastFile',tifPath);
            ...
            try
                ...
                if ~SI_Tiff
                    ...
                    if isfield(hdr.SI.hRoiManager, 'mroiEnable')
            ...
            function [roiDataInfo, roiData] = checkROIs(obj, roiDataSimple, file_mROI)
                ...
                for r = 1:numel(RoiData)
                   ...
                   [zs,zSortIdxs] = sort(roiDataSimple{r}.zs);
                   RoiData{r}.zs = zs;
                   
                   % reorder data
                   for chIdx = 1:numel(roiDataSimple{r}.imageData)
                       for volIdx = 1:numel(roiDataSimple{r}.imageData{chIdx})
                           roiDataSimple{r}.imageData{chIdx}{volIdx} = roiDataSimple{r}.imageData{chIdx}{volIdx}(zSortIdxs);
                       end
                   end
    ...
    methods
        ...
        function set.hMotionCorrector(obj,val)
            ...
            if obj.componentUpdateProperty('hMotionCorrector',val)
                ...
                obj.hCorrectNowListener = most.ErrorHandler.addCatchingListener(val,'correctNow',@(varargin)obj.correctMotion());
    ...
    methods (Access = private)
        ...
        function loadClassData(obj)
            ...
            function safeSetProp(propName)
                try
                    ...
                    most.ErrorHandler.logAndReportError(ME,sprintf('MotionManager: Could not restore property %s',propName));
                end
            end
        end
        ...
        function moveAxesAbsolute(obj,newMotionCorrectionVector)
            ...
            [moveFastZDynamic,fastZ_volt_offset,moveFastZStatic,fastZ_static_offset] = getFastZCorrection();
            ...
            if moveMotor
                motorPt = scanimage.mroi.coordinates.Points(obj.hSI.hCoordinateSystems.hCSReference,motor_d);
                obj.hSI.hMotors.move(motorPt);
            end
            ...
            if moveFastZStatic
                obj.hSI.hFastZ.positionTarget = obj.hSI.hFastZ.positionTarget + fastZ_static_offset;
            end
            ...
            function [moveMotor,motor_d] = getMotorCorrection()
                ...
                if correctionXYPossible && useMotorXY && ~isSlowZ % don't update motor position in slow Z stack acquisition
                    assert(obj.hSI.hMotors.isAligned);
                    motor_d(1:2) = dr(1:2); % motion vector
                    moveMotor = true;
                end
            ...
            function [moveFastZDynamic,fastZ_volt_offset,moveFastZStatic,fastZ_static_offset] = getFastZCorrection()
                moveFastZDynamic = 0;
                fastZ_volt_offset = 0;
                moveFastZStatic = 0;
                fastZ_static_offset = 0;
                
                useFastZ = dr(3)~=0 && strcmpi(obj.correctionDeviceZ,'fastz');
                if correctionZPossible && useFastZ
                    if obj.hSI.hFastZ.outputActive
                        % move FastZ dynamically by changing output waveform
                        hFastZ = ss.fastz;
                        dz = newMotionCorrectionVector(3);
                        
                        %%% coerce fastZ to allow actuator voltage range
                        ...
                        if ~isequal(newMotionCorrectionVector(3),obj.motionCorrectionVector(3))
                            ...
                            moveFastZDynamic = true;
                        end
                        
                    else
                        % move FastZ statically by changing position target
                        hFastZ = ss.fastz;
                        travelRange = sort(hFastZ.travelRange);
                        
                        dz = dr(3);
                        pos = obj.hSI.hFastZ.positionTarget + dz;
                        pos = min(max(pos,travelRange(1)),travelRange(2));
                        dz = pos - obj.hSI.hFastZ.positionTarget;
                        dr(3) = dz;
                        newMotionCorrectionVector(3) = obj.motionCorrectionVector(3) + dr(3);
                        
                        if dr(3)~=0
                            moveFastZStatic = true;
                            fastZ_static_offset = dr(3);
                        end
                    end
                end
            end
        ...
        function [lateralDevs,axialDevs] = getAvailableMotionCorrectionDevices(obj)
            ...
            isMotorAligned = obj.hSI.hMotors.isAligned;
            xGalvoAvailable = isGG || (isRGG && ~isempty(ss.scanners{2}));
            yGalvoAvailable = isGG || (isRGG && ~isempty(ss.scanners{3}));
            
            isSlowZwithMotor = obj.hSI.hStackManager.isSlowZ && obj.hSI.hStackManager.stackActuator==scanimage.types.StackActuator.motor;
            
            xMotorAvailable = isMotorAligned && ~isSlowZwithMotor;
            yMotorAvailable = isMotorAligned && ~isSlowZwithMotor;
            zMotorAvailable = ~isSlowZwithMotor;
            fastZAvailable  = ss.hasFastZ;
        ...
        function attachMotionEstimatorListeners(obj)
            ...
            for idx = 1:length(obj.hMotionEstimators)
                ...
                obj.hMotionEstimatorListeners = [obj.hMotionEstimatorListeners most.ErrorHandler.addCatchingListener(hMotionEstimator,'changed',@(src,evt)obj.notifyMotionEstimatorsChanged(src,evt))];
            end
        end
