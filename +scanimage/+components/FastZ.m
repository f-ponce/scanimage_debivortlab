classdef FastZ < scanimage.interfaces.Component & most.HasMachineDataFile & most.HasClassDataFile
    ...
    properties (Hidden,SetAccess=private)
        ...
        hStages = scanimage.components.motors.legacy.StageController.empty;   %Handle to FastZ hardware, may be a LSC object or a PI motion controller
    ...
    methods (Access = protected, Hidden)
        function mdlInitialize(obj)
            ...
            function znstInitFastZHardware(params)
                obj.hStages(i) = scanimage.components.motors.legacy.StageController(params,true,sprintf('FastZ_%d',i));
                ...
                obj.hScanners(i).sampleRateHz = min(200000,obj.maxSampleRate(i));
                ...
                nm = params.daqDeviceName;
				obj.isPXIs(i) = ~isRio(nm) && ~dabs.vidrio.rdi.Device.isRdiDeviceName(nm) && isPxi(nm);
            ...
            function params = znstInitFastZAO(params)
                ...
                if dabs.vidrio.rdi.Device.isRdiDeviceName(dqnm)
                    wk = obj.hSI.fpgaMap.isKey(dqnm);
                    if wk
                        hF = obj.hSI.fpgaMap(dqnm);
                        params.hFpga = hF.hFpga;
                    end
                    ...
                    obj.maxSampleRate(i) = obj.hAOTasks{i}.get('sampClkMaxRate');
                    obj.hAOTasks{i}.cfgSampClkTiming(min(200000,obj.maxSampleRate(i)), 'DAQmx_Val_FiniteSamps', 20);
            ...
            function znstInitFastZSlmHardware(params)
                ...
                obj.hStages(i) = scanimage.components.motors.legacy.StageController(params,false,name_);
    ...
    methods
        function val = get.zAlignment(obj)            
            id = obj.zScannerId();
            if isempty(id) || (numel(obj.zAlignments) < id)
                val = [];
            else
        ...
        function val = get.positionAbsoluteRaw(obj)
            if ~isempty(obj.hStage)
                try
                    ...
                    most.ErrorHandler.logAndReportError(ME,'Getting FastZ position failed.');
                    val = NaN;
                end
            else
                val = NaN;
        ...
        function set.positionAbsolute(obj,val)
            if isnan(val)
                % no-op, for ui update
            else
                error('Cannot set read-only property ''positionAbsolute''');
            end
        end
        ...
        function set.positionTarget(obj,v)
            ...
            obj.hSI.hCoordinateSystems.updateCSFocus();
            obj.hSI.hStackManager.updateZSeries();
            obj.positionAbsolute = NaN;
        end
...
%                 rg.scanStackAO(ss,zPowerReference,zs,zsRelative,waveform,fb,zActuator,sliceScanTime,[]);
% 
...
%         end
        ...
        function set.hStage(obj,val)
            obj.hStage = val;
            obj.positionTarget = obj.positionTarget;
        end
    end
    ...
    methods        
        ...
        function [toutput,desWvfm,cmdWvfm,tinput,respWvfm] = testActuator(obj)
            ...
            try
                ...
                [toutput, desWvfm, cmdWvfm] = scannerSet.zWvfm(obj.hSI.hScan2D.currentRoiGroup,zPowerReference,zs,zsRelative,fb,wvType);
    ...
    %% FRIEND METHODS
    methods (Hidden)
        function idx = getScannerId(obj,name)
            [tf, idx] = ismember(name,obj.scannerMapKeys);
            if tf
                idx = obj.scannerMapIds(idx);
            elseif ~isempty(obj.defaultScannerId)
                idx = obj.defaultScannerId;
            else
                idx = [];
            end
        end
        ...
        function hScanner = scanner(obj,name)
            ...
            hScanner = [];
            zScannerID = obj.getScannerId(name);
            
            if ~isempty(zScannerID)
        ...
        function liveUpdate(obj)
            if obj.numInstances && obj.active && (obj.enable || obj.enableFieldCurveCorr)
                if obj.sharingScannerDaq
    ...
    methods (Hidden, Access = protected)
        ...
        %   Runs code that starts with the global acquisition-start command
            if obj.outputActive
                obj.sharingScannerDaq = ~obj.isSlm && obj.hSI.hScan2D.controllingFastZ;
