classdef ConfigurationSaver < scanimage.interfaces.Component & most.HasClassDataFile
    ...
    properties (Constant, Hidden)
        ...
        USR_PROP_LIST_DEFAULT = {...
            ...
            'hConfigurationSaver.fastCfgAutoStartType'; 'hUserFunctions.userFunctionsUsr';
            'hDisplay.renderer'; 'hDisplay.channelsMergeEnable';'hDisplay.channelsMergeFocusOnly';
            };% 'hChannels.imagingSystemSettings'; % removed so that this will saved to cfg by default.
    end
    ...
    methods
        ...
        function usrSaveUsrAs(obj,fname,cfgfname,transientSave)
            ...
            lastPath = obj.getClassDataVar('lastUsrFile',obj.classDataFileName);
            if isempty(lastPath)
                lastPath = obj.getClassDataVar('lastConfigFilePath',obj.classDataFileName);
            ...
            if ~transientSave
                obj.setClassDataVar('lastUsrFile',usrFileName,obj.classDataFileName);
            end
        ...
        function cfgSaveConfigAs(obj,fname,transientSave)
            ...
            lastPath = obj.getClassDataVar('lastConfigFilePath',obj.classDataFileName);
            if isempty(lastPath)
                [lastPath,~,~] = fileparts(obj.getClassDataVar('lastUsrFile',obj.classDataFileName));
            ...
            if ~transientSave
                obj.setClassDataVar('lastConfigFilePath',fileparts(cfgfilename),obj.classDataFileName);
            end
        ...
        function cfgLoadConfig(obj,fname,transientLoad)
            ...
            try
                ...
                % These calls are needed to actually restore properties
                % relating to the imaging channels. usrLoadingUser calls
                % these but they were originally omitted from this function
                % which meant they failed to load when saving them to a cfg
                % file and loading it. They have since been moved as per
                % Gitlab issue #243: Channel cfg settings do not apply when loaded/bad tool tip
%                 obj.hSI.hChannels.saveCurrentImagingSettings();
%                 obj.hSI.hChannels.registerChannels(); 
        ...
        function fastCfgSetConfigFile(obj,idx,fname)
            ...
            lastPath = obj.getClassDataVar('lastFastConfigFilePath',obj.classDataFileName);
            if isempty(lastPath)
                lastPath = obj.getClassDataVar('lastConfigFilePath',obj.classDataFileName);
            ...
            obj.setClassDataVar('lastFastConfigFilePath',fileparts(cfgfilename),obj.classDataFileName);
    ...
    methods (Hidden, Access=private)
        ...
        function zprvEnsureClassDataFileProps(obj)
            obj.ensureClassDataFile(struct('lastUsrFile',''),obj.classDataFileName);
            obj.ensureClassDataFile(struct('lastConfigFilePath',''),obj.classDataFileName);
            obj.ensureClassDataFile(struct('lastFastConfigFilePath',''),obj.classDataFileName);
        end
