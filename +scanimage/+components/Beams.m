classdef Beams < scanimage.interfaces.Component & most.HasMachineDataFile
    ...
    properties (SetObservable)
        ...
        powers = [];                % [Numeric] 1xN array containing power values for each beam
    ...
    properties (Hidden,SetAccess=private)
        ...
        hCalInputTask = dabs.vidrio.ddi.GenericTask.empty(1,0);
        hCalOutputTask = dabs.vidrio.ddi.GenericTask.empty(1,0);
        hDaqDevice;
        ...
        function obj = Beams(hSI)
            ...
            try
                ...
                for i = 1:obj.numInstances
                    ...
                    elseif dabs.vidrio.rdi.Device.isRdiDeviceName(dqnm)
                        s = dabs.vidrio.rdi.Device.getDriverInfo();
                        assert(obj.hSI.mdfData.simulated || (s.numDevices > str2double(dqnm(5:end))), 'Invalid RDI device.');
                        obj.hDaqDevice{i} = dqnm;
                    else
                    ...
                    % displayNames
                    nms = vertcat(obj.displayNames,arrayfun(@(x)sprintf('DAQ %d Beam %d',i,x),obj.daqBeamIDs{i},'UniformOutput',false));
                    if isempty(daqData.displayNames)
                        obj.displayNames = nms;
                    else
                        emps = cellfun(@isempty,daqData.displayNames);
                        daqData.displayNames(emps) = nms(emps);
                ...
                defaultPower = 10;
                obj.powers = repmat(defaultPower, 1, obj.totalNumBeams);
            catch ME
                most.ErrorHandler.logAndReportError(ME,sprintf('Error occurred while initializing beam control. Incorrect MachineDataFile settings likely cause. Error message:\n%s',ME.message));
                obj.ziniDisableBeamsFeature();
            end
    ...
    methods
        function val = get.acqLengthConstants(obj)
            ...
            lc = obj.beamComputeOverrideLzs(lc);
...
%                 end
        ...
        function set.powers(obj,val)
            ...
            if obj.internalSetFlag || obj.componentUpdateProperty('powers',val)
                ...
                val(isnan(val)) = obj.powers(isnan(val));
    ...
    methods 
        ...
        function beamsCalibrate(obj,beamIDs,showShutterWarning)
            ...
            if obj.componentExecuteFunction('beamsCalibrate',beamIDs,showShutterWarning)
                ...
                for i = unique(obj.globalID2Daq(beamIDs))
                    ...
                    try
                        if ~isempty(obj.mdfData.beamDaqs(i).calOpenShutterIDs) && ~isempty(obj.hCalInputTask(i).channels)
                            for bIdx = globalIDs
            ...
            if obj.componentExecuteFunction('beamsOn',varargin{:})
                ...
                for i = 1:obj.numInstances
                    if (isempty(obj.activeBeamDaqID) || i ~= obj.activeBeamDaqID) && ~any([obj.externalBeamDAQReq.daq] == i)
                        ...
                        obj.hPark{i}.setChannelOutputValues(voltages);
            ...
            if obj.componentExecuteFunction('beamsOff')
                ...
                for i = 1:obj.numInstances
                    if (isempty(obj.activeBeamDaqID) || i ~= obj.activeBeamDaqID) && ~any([obj.externalBeamDAQReq.daq] == i)
                        ...
                        beamIDs = obj.daqBeamIDs{i};
                        obj.updateBeamStatusFrac(beamIDs,NaN);
                        obj.hPark{i}.setChannelOutputValues(obj.zprpBeamsPowerFractionToVoltage(beamIDs,zeros(1,numel(beamIDs))));
        ...
        function offset = beamsMeasureCalOffset(obj,beamIdx,tfWriteToMDF)
            ...
            if obj.componentExecuteFunction('beamsMeasureCalOffset',beamIdx,tfWriteToMDF)
                ...
                beamCalTask = obj.hCalInputTask(beamIdx);
                if isempty(beamCalTask.channels)
                ...
                beamCalTask.stop();
                beamCalTask.sampleRate = obj.calibrationOutputRate;
                beamCalTask.sampleMode = 'finite';
                beamCalTask.samplesPerTrigger = obj.calibrationOffsetNumSamples;
                beamCalTask.startTrigger = '';
                beamCalTask.allowRetrigger = false;
                ...
                beamCalTask.start();
                data = beamCalTask.readInputBuffer(obj.calibrationOffsetNumSamples, 1.1 * obj.calibrationOffsetNumSamples/obj.calibrationOutputRate);
                beamCalTask.stop();
        ...
        function acqLzs = beamComputeOverrideLzs(obj,acqLzs)
            ...
            if obj.hSI.hStackManager.overrideLZs                
                ...
                for c = 1:Nbeam
                    Pratio = obj.hSI.hStackManager.stackEndPower(c)/obj.hSI.hStackManager.stackStartPower(c);
        ...
        function updateStaticBeams(obj)
            ...
            for i = 1:obj.numInstances
                if (isempty(obj.activeBeamDaqID) || i ~= obj.activeBeamDaqID) && ~any([obj.externalBeamDAQReq.daq] == i)
                    ...
                    obj.hPark{i}.setChannelOutputValues(beamPowerVoltages(obj.daqBeamIDs{i}));
                    obj.updateBeamStatusFrac(obj.daqBeamIDs{i},beamPowerFracs(obj.daqBeamIDs{i}))
                end
            end
        end
        
        function s = scanner(obj,daqId,sampleRate,linePhase,beamClockDelay,beamClockExtend,beamIds)
            if obj.numInstances > 0 && ~isempty(daqId)
                ...
                isBoundedStack = obj.hSI.hStackManager.enable ...
                              && obj.hSI.hStackManager.stackDefinition == scanimage.types.StackDefinition.bounded ...
                              && obj.hSI.hStackManager.stackStartEndPointsDefined;

                if isBoundedStack
                  powers_ = obj.hSI.hStackManager.stackStartPower(beamIds);
                else
                  powers_ = obj.powers;
                end
                
                s = scanimage.mroi.scanners.Beams(beamIds,...
                        sampleRate,...
                        powers_,...
                        obj.powerLimits,...
                        obj.flybackBlanking,...
                    ...
                    if ~isempty(obj.activeBeamDaqID)
                        ...
                        if framesToWrite > 0
                            ...
                            if ~obj.hSI.mdfData.simulated
                                ...
                                obj.hTask{obj.activeBeamDaqID}.writeAnalogDataAsync(double(aoVolts),[],[],[],@(src,evt)obj.updateBeamBufferAsyncCallback(src,evt));
                            end
...
%             voltage = interp1(linspace(1/obj.calibrationLUTSize,1,obj.calibrationLUTSize),obj.calibrationLUT(:,beamIdx),lutx);
    ...
    methods (Hidden)
        ...
        function powers = zprvBeamsDepthPowerCorrection(obj,beamIdx,powers,zReference,z,lz)
            if obj.numInstances > 0
                if(isempty(obj.pzCustom{beamIdx}))
                    powers = (powers.*exp((z-zReference)./lz));
                elseif(isa(obj.pzCustom{beamIdx}, 'function_handle'))
                    powers = obj.pzCustom{beamIdx}(powers,zReference,z,lz);
        ...
        function rearm(obj)
            if ~isempty(obj.hSI.hScan2D.beamDaqID)
                assert(obj.active);
                obj.beamBufferNeedsUpdateAsync = false;
...
%                 set(obj.hTask{obj.activeBeamDaqID},'sampClkTimebaseSrc','');
        ...
        function [tfSuccess,beamCalVoltage,beamVoltage] = zprvBeamsGetCalibrationData(obj,beamIdx)
            ...
            if isempty(obj.hCalInputTask(beamIdx).channels)
                tfSuccess = true;
                beamCalVoltage = [];
                znstCleanup(false);
                return;
            ...
            % Prepare tasks
            hInputTask = obj.hCalInputTask(beamIdx);
            hInputTask.abort()
            hInputTask.clearSyncedTasks();
            hInputTask.sampleMode = 'finite';
            hInputTask.startTrigger = '';
            hInputTask.triggerOnStart = true;
            hInputTask.allowRetrigger = true;
            hInputTask.autoStartStopSyncedTasks = false;
            hInputTask.allowEarlyTrigger = false;
            hInputTask.sampleRate = obj.calibrationOutputRate;
            hInputTask.samplesPerTrigger = NbeamVoltage;
            
            hOutputTask = obj.hCalOutputTask(beamIdx);
            hOutputTask.abort()
            hOutputTask.syncTo(hInputTask);
            hOutputTask.writeOutputBuffer(beamVoltage);
            ...
            try % Why is this try/catch necessary?
                ...
                for c = 1:obj.calibrationNumPasses
                    ...
                    hInputTask.start();
                    beamCalVoltage(:,c) = hInputTask.readInputBuffer(NbeamVoltage, 1.5 * NbeamVoltage / obj.calibrationOutputRate);
                    hInputTask.stop();
    ...
    methods (Access=protected, Hidden)
        function mdlInitialize(obj)
            if obj.numInstances > 0
                for i = 1:obj.numInstances
                    device = obj.hDaqDevice{i};
                    taskName = sprintf('Beam DAQ %d Modulation',i);
                    daqData = obj.mdfData.beamDaqs(i);
                    
                    if ischar(device)
                        hF = obj.hSI.fpgaMap(device);
                        device = hF.hFpga;
                        obj.hDaqDevice{i} = device;
                        obj.hTask{i} = [];
                    else
                        % create task for active acquisition
                        obj.hTask{i} = dabs.ni.rio.fpgaDaq.fpgaDaqAOTask.createTaskObj(taskName, device);
                    end
                    
                    % create static task
                    obj.hPark{i} = dabs.vidrio.ddi.AoTask(device, [taskName ' Park']);
                    
                    for j=1:obj.daqNumBeams(i)
                        idString = obj.displayNames{obj.daqBeamIDs{i}(j)};
                        
                        obj.hPark{i}.addChannel(daqData.chanIDs(j),sprintf('%s Park',idString));
                        ...
                        obj.hCalInputTask(end+1) = dabs.vidrio.ddi.AiTask(device, [idString ' Calibration Input']);
                        obj.hCalOutputTask(end+1) = dabs.vidrio.ddi.AoTask(device, [idString ' Calibration Output']);
                        if ~isnan(daqData.calInputChanIDs(j))
                            obj.hCalInputTask(end).addChannel(daqData.calInputChanIDs(j),[idString ' Calibration Input']);
                            obj.hCalOutputTask(end).addChannel(daqData.chanIDs(j),[idString ' Calibration Output']);
                        end
                    end
                    ...
                    if ~isempty(obj.hTask{i})
                        ...
                        assert(~isempty(supportedSampleRate),'DAQ Device %s does not support hardware timed sampling',obj.hDaqDevice{i}.deviceName);
                    ...
                    elseif isa(device,'dabs.vidrio.rdi.Device')
                        obj.maxSampleRate(i) = 1e6;
                    else
                        obj.maxSampleRate(i) = nan;
                    end
                end
    ...
    methods (Hidden,Access = protected)
        ...
        %   Runs code that starts with the global acquisition-start command
            if ~isempty(obj.hSI.hScan2D.beamDaqID)
                ...
                obj.activeBeamDaqID = obj.hSI.hScan2D.beamDaqID;
                
                obj.sharingScannerDaq = isempty(obj.hTask{obj.activeBeamDaqID}) || (isfield(obj.hSI.hScan2D.mdfData, 'deviceNameGalvo') &&...
                    strcmp(obj.mdfData.beamDaqDevices{obj.activeBeamDaqID}, obj.hSI.hScan2D.mdfData.deviceNameGalvo));
                ...
                obj.updateBeamStatusFrac(beamIDs,NaN);
                obj.hPark{obj.activeBeamDaqID}.setChannelOutputValues(voltages);
        ...
        %   Runs code that aborts with the global acquisition-abort command
            ...
            obj.beamBufferNeedsUpdateAsync = false;
            if ~obj.sharingScannerDaq && ~isempty(obj.activeBeamDaqID) && ~isempty(obj.hTask{obj.activeBeamDaqID})
                obj.hTask{obj.activeBeamDaqID}.abort();
            end
            obj.beamBufferUpdatingAsyncNow = false;
            obj.activeBeamDaqID = [];
