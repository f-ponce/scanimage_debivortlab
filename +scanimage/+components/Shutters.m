...
% Shutters     Functionality for managing shutters and shutter state transitions.
    properties (SetObservable,SetAccess = private,Transient)
        ...
        lastTransition = [];                               % timestamp of last transition for each shutter
    end
    ...
    properties (Hidden, SetAccess=protected)
        ...
        mdlHeaderExcludeProps = {'status','lastTransition'};
    end
        ...
        function mdlInitialize(obj)
            try
                ...
                for i = 1:numShutters
                    ...
                    elseif dabs.vidrio.rdi.Device.isRdiDeviceName(dev)
                        hFpga = obj.hSI.fpgaMap(dev).hFpga;
                        hFpga.setDioOutput(chan,~obj.mdfData.shutterOpenLevel(i));
                        obj.hShutterTransitionFuncs{i} = @(openTF)hFpga.setDioOutput(chan,openTF == obj.mdfData.shutterOpenLevel(i));
                    else
                ...
                most.ErrorHandler.logAndReportError(ME, sprintf('Error occurred during shutter initialization. Incorrect MachineDataFile settings likely cause. Message:\n%s', ME.message));
                numShutters = 0;
                obj.status = [];
            end
            
            obj.numInstances = numShutters;
            obj.lastTransition = repmat(tic(),1,numShutters);
            obj.shuttersTransitionAll(false);
    ...
    methods
            ...
            if obj.componentExecuteFunction('shuttersTransition',shutterIDs,openTF,applyShutterOpenTime)
                ...
                for i = 1:numel(shutterIDs)
                    try
                        ...
                        oldStatus = obj.status(shutterIDs(i));
                        newStatus = openTF(i); % only is set if transition successful
                        obj.status(shutterIDs(i)) = newStatus;
                        if oldStatus ~= newStatus
                            obj.lastTransition(shutterIDs(i)) = tic();
                        end
                    catch ME
                ...
                if applyShutterOpenTime
                    obj.waitForTransitionComplete();
                end
            end
        end
        ...
        function shuttersTransitionAll(obj, openTF, applyShutterOpenTime)
            ...
            if nargin < 3 || isempty(applyShutterOpenTime)
                applyShutterOpenTime = [];
            end
            
            shutterIDs = 1:obj.numInstances;
            obj.shuttersTransition(shutterIDs,openTF,applyShutterOpenTime);
        end
        
        function [allComplete,waitTime] = isTransitionComplete(obj)
            ...
            elapsedTime = arrayfun(@(t)toc(t),obj.lastTransition);
            waitTimes = max(obj.mdfData.shutterOpenTime - elapsedTime, 0);
            waitTime = max(waitTimes);
            
            allComplete = waitTime<=0;
        end
        
        function waitForTransitionComplete(obj)
            [allComplete,waitTime] = isTransitionComplete(obj);
            
            if ~allComplete
                most.idioms.pauseTight(max(waitTime));
            end
        end
    end
