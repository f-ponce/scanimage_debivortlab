classdef Scan2D < scanimage.interfaces.Component & most.HasClassDataFile 
    ...
    properties (SetObservable, Transient, Dependent)
        ...
        %logFilePath;  % Grouped with other logging parameters so path will save to cfg/usr (JLF MAY2019)  % string, path where tif files will be written
    ...
    properties (Hidden, SetAccess={?scanimage.interfaces.Class})
        ...
        hSlmScan;
        hDataScope;                          % handle to DataScope object
    end
    ...
    properties (Hidden, Dependent, SetObservable)
        ...
        stimScannerset;                         % scanimage.mroi.scannerset.ScannerSet 
    end
...
%     end
    ...
    %% LIFECYCLE
    methods
        function obj = Scan2D(hSI,name,legacymode)
            if nargin < 3 || isempty(legacymode)
                legacymode = false;
            end
            ...
            obj.simulated = hSI.mdfData.simulated || obj.mdfData.simulated;
    ...
    methods            
        ...
        function val = get.stimScannerset(obj)
            try
                val = obj.getScannerset(false,true);
            catch
                val = obj.accessScannersetPostGet(obj);
            end
...
%             end
        ...
        function val = get.scannerToRefTransform(obj)
            val = obj.scannerToRefTransform;
            val = obj.accessScannerToRefTransformPostGet(val);
        end
        ...
        function set.hDataScope(obj,val)
            if ~isempty(val)
                assert(isa(val,'scanimage.components.scan2d.interfaces.DataScope'),'Not a valid scanimage.components.scan2d.interfaces.DataScope');
                assert(isscalar(val),'Not a valid scanimage.components.scan2d.interfaces.DataScope');
                assert(most.idioms.isValidObj(val),'Not a valid scanimage.components.scan2d.interfaces.DataScope');
            end
    ...
    % default implementations of some access methods. Overload if needed
    ...
    methods (Hidden, Access = protected)
        ...
        function safeAbortDataScope(obj)
            try
                if ~isempty(obj.hDataScope) && most.idioms.isValidObj(obj.hDataScope)
    ...
    methods(Static)
        function s = scan2DPropAttributes()
            ...
            s.beamClockDelay          = struct('Classes','numeric','Attributes',{{'scalar','finite','nonnegative'}});
