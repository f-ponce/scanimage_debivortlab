...
%classdef WSConnectorModel < most.Model
    ...
    methods
        function obj = WSConnector(hSI)
            ...
            obj.hListeners = most.ErrorHandler.addCatchingListener(obj.hSI.hUserFunctions,'usrFileSaved',@obj.saveUserFile);
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hSI.hUserFunctions,'usrFileLoaded',@obj.loadUserFile);
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hSI.hUserFunctions,'cfgFileSaved',@obj.saveConfigurationFile);
            obj.hListeners(end+1) = most.ErrorHandler.addCatchingListener(obj.hSI.hUserFunctions,'cfgFileLoaded',@obj.loadConfigurationFile);
        end
        ...
        function zzPollTimerFcn(obj,src,evnt)            
            ...
            if exist(obj.siCmdFullPath,'file') 
                ...
                if isempty(MEtemp)
                    ...
                    fprintf(fid,'ERROR\n');
                    fclose(fid);
                    most.ErrorHandler.logAndReportError(MEtemp);
                end
            end
        end
