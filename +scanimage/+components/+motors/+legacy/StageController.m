...
%
    ...
    methods
        function obj = StageController(motorParams,analogOnly,name)
            ...
            try
                ...
                most.ErrorHandler.logAndReportError(ME,['Error initializing stage controller: ' ME.message]);
            end
        end
    ...
    methods (Hidden)
        ...
        function newv = zprpValidateTwoStepPropVals(obj,v)
            ...
            newv = struct();
            for f = scanimage.components.motors.legacy.StageController.twoStepPropNames(:)'
                if isfield(v,f{1})
                    newv.(f{1}) = v.(f{1});
                end
            end
    ...
    methods
        function reinitMotor(obj)
            ...
            try
                ...
                regInfo = scanimage.components.motors.legacy.MotorRegistry.getControllerInfo(lower(type));
                ...
                if ~isfield(lscArgs,'numDeviceDimensions')
                    lscArgs.numDeviceDimensions = regInfo.NumDimensions;
                end
                ...
                if isa(obj.hLSC,'dabs.interfaces.LSCAnalogOption')
                    ...
                    if isfield(obj.motorParams, 'hFpga')
                        obj.hLSC.controlDevice = obj.motorParams.hFpga;
                    end
                ...
                obj.numDeviceDimensions = obj.hLSC.numDeviceDimensions;
            catch ME
                most.idioms.safeDeleteObj(obj.hLSC);
                obj.lscErrPending = true;
                ME.rethrow();
