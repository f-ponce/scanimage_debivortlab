    ...
    methods (Hidden)
        function obj = SIController(hModel)
            ...
            visibleGuis = {'mainControlsV4' 'configControlsV4' 'imageControlsV4'...
                'channelControlsV4' 'scanimage.guis.statuswindow.StatusWindow' 'scanimage.guis.MotorControls'};
            hiddenGuis = {'powerControlsV4'...
                'fastConfigurationV4' 'userFunctionControlsV4' 'triggerControlsV5' 'userSettingsV4'...
                'photostimControlsV5' 'pmtControlsV5' 'powerBoxControlsV4' 'scanimage.guis.WaveformControls'...
                ...
                'scanimage.guis.StackControls' 'scanimage.guis.ResonantCalibrator'};
        ...
        function initialize(obj,usr,hidegui)
            ...
            if most.idioms.isValidObj(obj.hConfigEditor)
                ...
                obj.hConfigEditorLis = most.ErrorHandler.addCatchingListener(obj.hConfigEditor, 'mdfUpdate', @obj.mdfUpdate);
            end
...
%             drawnow expose % otherwise the main gui is not always moved to the correct position
            ...
            most.gui.tetherGUIs(obj.hGUIs.configControlsV4,obj.hGUIs.ResonantCalibrator,'righttop',obj.WINDOW_BORDER_SPACING);
            ...
            most.gui.tetherGUIs(obj.hGUIs.MotorControls,obj.hGUIs.pmtControlsV5,'bottomleft',obj.WINDOW_BORDER_SPACING);
...
%             end
        ...
        function ziniMainControls(obj)
            ...
            disabledControls = [disabledControls {'cbEnableMroi' 'cbIntegration' 'pbPhotostim' 'pbEditRoiGroup' 'pbIntegration' 'mnu_View_PhotostimControls' 'mnu_View_AlignmentControls' 'mnu_View_IntegrationControls' 'mnu_View_WaveformControls' 'mnu_View_SlmControls' 'mnu_View_LaserTriggerScope' 'mnu_View_PhotonCountingCtrls'}];
...
%                 end
                ...
                obj.hPmtListener = most.ErrorHandler.addCatchingListener(obj.hModel.hPmts, 'statusChanged', @obj.changedPmtsStatus);
        ...
        function changeFastZCfg(obj)
            if obj.hModel.hFastZ.hasFastZ && obj.hModel.hFastZ.hasFieldCurveCorrection
        ...
        function updateScanType(obj,varargin)
            switch obj.hModel.hRoiManager.scanType
                case 'frame'
                    ...
                    set(obj.hGUIData.mainControlsV4.framesTotal,'Enable',obj.tfMap(~obj.hModel.hFastZ.enable || strcmp(obj.hModel.hFastZ.waveformType, 'step')));
                    if obj.hModel.hFastZ.enable && isinf(obj.hModel.hStackManager.framesPerSlice)
        ...
        function changedScanMode(obj,~,~)
            ...
            switch obj.hModel.hScan2D.scanMode
                case 'resonant'
                    ...
                    set(obj.hGUIData.configControlsV4.pbFovCal, 'Visible', 'on');
                ...
                case 'linear'
                    ...
                    set(obj.hGUIData.configControlsV4.pbFovCal, 'Visible', 'off');
...
%             end
            ...
            if obj.hModel.hStackManager.overrideLZs
                powerReference = obj.hModel.hStackManager.stackStartPower(obj.beamDisplayIdx); % in percent
                zReference = obj.hModel.hStackManager.zPowerReference;
            else
                powerReference = obj.hModel.hBeams.powers(obj.beamDisplayIdx); % in percent
                zReference = obj.hModel.hStackManager.zPowerReference;
            end
...
%             if obj.hModel.hStackManager.enable && obj.hModel.hStackManager.stackDefinition == scanimage.types.StackDefinition.bounded
%                 if obj.hModel.hStackManager.stackStartEndPointsDefined & obj.hModel.hStackManager.useStartEndPowers
%                     powersNominal = obj.hModel.hStackManager.stackStartPower(obj.beamDisplayIdx);
%                 end
%             end
            ...
            powers = obj.hModel.hBeams.zprvBeamsDepthPowerCorrection(obj.beamDisplayIdx, powerReference, zReference, zs(:), lz);
...
%                 obj.hModel.hStackManager.framesPerSlice = 1;
                ...
                if most.idioms.isValidObj(obj.hFastZTuneFig)
                    if obj.hModel.hFastZ.hasFastZ
                        ...
                        if nargin < 3
                            ...
                            zPowerReference = 0;
                            [totput, desWvfm, cmdWvfm] = scannerSet.zWvfm(obj.hModel.hScan2D.currentRoiGroup,zPowerReference,zs,zsRelative,fb,wvType);
                            respWvfm = [];
                        end
        ...
        function openConfigEditor(obj)
            try
                ...
                most.ErrorHandler.logAndReportError(ME,'Failed to launch Configuration Editor.');
            end
        end
        ...
        function changePmtsGains(obj,pmtNum,val)
            obj.hGUIs.pmtControlsV5.CurrentObject = [];
            obj.gGains(:) = nan;
        ...
        function changedPmtsStatus(obj,~,~)
            ...
            if any(gains ~= obj.gGains)
                obj.gGains = gains;
                hCurrentObj = obj.hGUIs.pmtControlsV5.CurrentObject;
                for i = 1:min(numel(gains),numControlsOnGui)
                    etTag = sprintf('etPmt%dGain',i);
                    ctl = obj.hGUIData.pmtControlsV5.(etTag);
                    if isempty(hCurrentObj) || (ctl ~= hCurrentObj)
                        set(ctl,'String',gains(i));
                    end
                end
                tfDr = true;
            end
