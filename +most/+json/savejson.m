...
if(jsonopt('Compact',0,opt)==1)
    ...
    whitespaces=struct('tab',jsonopt('tab',sprintf('\t'),opt),'newline',sprintf('\n'),'sep',sprintf(',\n'));
end
...
elseif isa(item, 'function_handle');
    txt=str2json(name,func2str(item),level,varargin{:});
else
...
if numel(item) == 0 %empty object
    ...
    % visible properties
    if ismethod(item,'saveobj')
        st = arrayfun(@saveobj,item);
    else
