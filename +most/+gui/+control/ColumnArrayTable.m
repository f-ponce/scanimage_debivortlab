classdef ColumnArrayTable < most.gui.control.PropControl
    ...
    properties
        ...
        rowHeadingLabels; % optional row headings as determined by 'column 0' labels defined in the controller.
    ...
    methods 
        ...
        function init(obj,metadata)
            ...
            % * 'type': Prop is a string array which is specially processed
            %    by columnArrayTable. Based on the string value, the user
            %    can pass certain display options to the columnArrayTable.
            %
                ...
                if colIdx == 0
                    switch md.format
                        case 'cellstr'
                            %do nothing for now.
                        otherwise
                            fprintf('Error: Only cellstr allowed for row label props.\n');
                    end
                else
    ...
    methods
        ...
        function encodeFcn(obj,propname,newVal)
            ...
            %run encodefcn on everything but row labels (colIdx = 0)
            if colIdx > 0
                fmt = obj.colFormats(colIdx);
                ...
                if numel(newVal) > size(data,1)
                   % Truncate newVal data if it doesn't fit in the table.
                   newVal = newVal(1:size(data,1));
%                 elseif numel(newVal) < size(data,1)
%                    % Truncate newVal data if it doesn't fit in the table.
%                    data = data(1:numel(newVal),:);
                end
...
%                      assert(numel(newVal)==size(data,1));
                ...
                % This is the row label of the ColumnArrayTable. Store it in the obj.rowHeadingLabels prop.
                obj.rowHeadingLabels = newVal;
                % Trigger a redraw
                obj.resize(numel(newVal));
            end
        end
    ...
    methods (Access=private)
        ...
        function labelTableRows(obj)
            if ~isempty(obj.rowHeadingPattern)
                ...
                % If there is no rowHeadingPattern, then name the rows based on the column '0' labels provided by the user.
                nrows = size(get(obj.hTable,'Data'),1);
                rowNames = obj.rowHeadingLabels;
                set(obj.hTable,'RowName',rowNames);
            end
        end
    end
    ...
    methods (Static,Access=private)
        ...
        function v = encodeValue(v,colfmt,n)
            %EKK - 3/27/15 - modified to allow nans.
            switch colfmt.format
                case 'numeric'
                    assert(isnumeric(v));
                ...
                case 'logicalindices'
                    %assert(isnumeric(v));
                    %what is the point of this code exactly? to go from
                    %numeric to a cell?
                    newcol = false(n,1);
                    newcol(v) = true;
                    v = num2cell(newcol);
