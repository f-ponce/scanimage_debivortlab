...
[x,y] = most.mimics.getpts(ax,'numberOfPoints',1);
...
if ~ishandle(fig)
    most.idioms.dispError('%s - getRectFromAxes: figure handle is no longer valid.\n%s\n', datestr(now), getStackTraceString);
    return;
end
...
try
    ...
    most.idioms.dispError('Error processing windowButtonUpFcn:\n%s\n', getLastErrorStack);
    uiresume;
end
