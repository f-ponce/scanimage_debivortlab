...
if ~ishandle(fig)
    most.idioms.dispError('%s - getPointsFromAxes: figure handle is no longer valid.\n%s\n', datestr(now), getStackTraceString);
    return;
end
...
switch lower(eventdata.Key)
    case 'backspace'
        try
            ...
            most.idioms.dispError('Error processing deletePoint:\n%s\n', getLastErrorStack);
            uiresume;
        end
    case 'delete'
        try
...
try
    ...
    most.idioms.dispError('Error processing addPoint:\n%s\n', getLastErrorStack);
    uiresume;
end
