...
args = most.util.filterPVArgs(varargin,VALIDPROPS);
props = args(1:2:end);
vals = args(2:2:end);
...
if s.AllowEmptyDouble && most.idioms.isEmptyDouble(val) 
    return;
end
...
if iscellstr(options)
    assert(most.idioms.isstring(val) && ismember(val,options),EID,EMSG);
    cls = 'char';
