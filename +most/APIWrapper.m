classdef APIWrapper < most.HasClassDataFile %& most.DClass
    ...
    properties (Constant,Hidden)
        matlabVerLessThan81 = verLessThan('matlab','8.1');
    end
    ...
    properties (Constant, Hidden)
       apiDLLDefaultPath = fullfile(most.idioms.startPath,'windows','system32'); %This is default location for both Win32 and x64 platforms
    end
    ...
    methods
        function obj = APIWrapper()
            ...
            function ziniSetDefaultValues()
                ...
                defaultValMap('apiAuxFile1Paths') = most.util.className(obj,'classPrivatePath');
                defaultValMap('apiAuxFile2Names') = '';
                defaultValMap('apiAuxFile2Paths') = most.util.className(obj,'classPrivatePath');
                
                %Handle apiDLLPath default -- can be specified in MDF
                if ~isa(obj,'most.HasMachineDataFile') || ~isfield(obj.mdfData,'apiDLLPath')
                    defaultVal = '';
                else
                ...
                switch obj.apiHeaderRootPath
                    case 'class'
                        obj.apiHeaderRootPath = most.util.className(obj,'classPrivatePath');
                    case 'package'
                        obj.apiHeaderRootPath = most.util.className(obj,'packagePrivatePath');
                    otherwise
                        % "real path", no-op.
                end
            ...
            function ziniInitializeAPIDataFile()
                ...
                function ziniUpdateAPIDataFile()
                    ...
                    try
                        ...
                        obj.accessAPIDataVarStatic(class(obj),[],true); % force reloading the cache
                    catch ME
                        fprintf(1,'\n');
                        ME.rethrow();
                    end
    ...
    methods
        ...
        function val = get.apiDataFullFileName(obj)
            ...
            if isempty(obj.apiCachedDataPath)
                cachedDataPath = most.util.className(obj,'classPrivatePath');
            else
    ...
    methods (Hidden)
        ...
        function varargout = apiCallFiltered(obj,funcName,filteredResponseCodes,varargin)
            ...
            try
                if ~obj.matlabVerLessThan81 %2013a or later
        ...
        function apiProcessErrorResponseCode(obj, responseCode, funcName)
            ...
            if warningOnly
                ...
                warning([most.util.className(obj) ':APICallWarning'],errorString);
                warning(s.state, 'backtrace');
            else
                ME = MException([most.util.className(obj) ':APICallError'],errorString);
                ME.throwAsCaller();
            end
        ...
        function smartLoadLibrary(obj)
            ...
            try
                ...
                if ~libisloaded(obj.apiDLLName)
                    ...
                    DLLName = obj.apiDLLName;
                    
                    [headerPath,headerFile,headerExt] = fileparts(obj.apiHeaderFileName);
                    
                    if strcmpi(headerExt,'.h')
                        loadlibrary(fullfile(DLLPath,DLLName),obj.apiHeaderFileName);
                    elseif any(strcmpi(headerExt,{'.m' '.p'}))
                        currPath = cd;
                        try
                            cd(headerPath);
                            loadlibrary(fullfile(DLLPath,DLLName),str2func(headerFile));
                            cd(currPath);
                        catch ME
                            cd(currPath);
                            ME.rethrow();
    ...
    methods (Static, Access=protected)
        ...
        function val = apiCachedDataPathStatic(className)
            ...
            if isempty(apiCachedDataPathSpec)
                val = most.util.className(className,'classPrivatePath');
            else
    ...
    methods (Static, Hidden)
        ...
        function showLoadLibraryWarnings()
            most.DClass.setClassDataVarStatic(mfilename('class'),'showLoadLibraryWarnings',true);
        end
        
        function hideLoadLibraryWarnings()
            most.DClass.setClassDataVarStatic(mfilename('class'),'showLoadLibraryWarnings',false);
        end
        
        
        function val = accessAPIDataVarStatic(className,varName,forceReload)
            ...
            if nargin < 3 || isempty(forceReload)
                forceReload = false;
            end
            ...
            if ~APIDataStructMap.isKey(className) || forceReload
