classdef Model < handle
    ...
    methods (Access=protected, Hidden)
        function mdlInitialize(obj)
            ...
            obj.processMdlPropAttributes(); % reprocess mdlPropAttributes to capture added Scan2Ds
    ...
    methods (Access=private, Hidden)
        ...
        function processMdlPropAttributes(obj)
            propNames = fieldnames(obj.mdlPropAttributes);
            
            for i=1:length(propNames)
                propName = propNames{i};
                currPropMD = obj.mdlPropAttributes.(propName);
                obj.mdlPropAttributes.(propName) = sanitizeModelAttributes(propName,currPropMD);
            end
            
            %%% Nested function
            function currPropMD = sanitizeModelAttributes(propName,currPropMD)
                ...
                %Processing Step 2: Store list of props specifying submodels (ie. Classes constraint is 'most.Model')s
                if isfield(currPropMD,'Classes') && ismember('most.Model',cellstr(currPropMD.Classes))
                    assert(isscalar(cellstr(currPropMD.Classes)),'Property Class constraint to most.Model cannot be combined with other Class constraints or property attribute specification. Violated for property ''%s''.',propName);
                    
                    if ~ismember(propName,obj.mdlSubModelClasses) && ~isempty(obj.(propName))
                        obj.mdlSubModelClasses{end+1} = propName;
                        obj.(propName).mdlHParent = obj;
                        currPropMD = obj.(propName).mdlPropAttributes;
                    end
                end
            end
        end
            ...
            for i=1:length(propAttribs)
                ...
                if ~isempty(k)
                    ...
                    for j=1:length(dependsOnList)
                        ...
                        listenerArray = [listenerArray most.ErrorHandler.addCatchingListener(hDpdObj,dpdPropName,'PostSet',@(src,evnt)znstDummySet(src,evnt,hObj,propName))]; %#ok<AGROW> %TMW: Somehow it's not allowed to use trick of growing array from end to first, with first assignment providing the allocation.
                    end
            ...
            if isfield(obj.mdlDependencies, ['hSI___' SubMdlName])
                ...
                for i = 1:numel(bindTos)
                    ...
                    for j = 1:numel(vars.(bindTo))
                        ...
                        hL = most.ErrorHandler.addCatchingListener(hDpdObj,bindTo,'PostSet',@(src,evnt)znstDummySet(src,evnt,bindData.mdl,bindData.var));
    ...
    methods (Hidden)
        ...
        function handle = mdlSetPropListenerFcn(obj,pname,event,callbackFcn)       
            ...
            % try
            if isempty(mdlName)
                handle = most.ErrorHandler.addCatchingListener(obj,pnameShort,event,callbackFcn);
            else
                hObj = eval(['obj.' mdlName]);                                
