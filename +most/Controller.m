classdef Controller < most.DClass & most.HasClassDataFile & dynamicprops
    ...
    properties (SetAccess=private)
        hModel;     % handle to root model
        guiNames;   % cell array of gui names
        ...
        hGuiClasses = struct(); % Struture effecting Map of guiNames to objects that implement programmatic GUIs
    ...
    properties (Constant,Hidden)
        graphics2014b = most.idioms.graphics2014b; % cached for performance
    end
    ...
    methods
        function obj = Controller(hModel,guiNames,guiNamesInvisible)
            ...
            if obj.graphics2014b
                obj.hAuxGUIs = gobjects(1,0);
                obj.hGUIsArray = gobjects(1,0);
            else
                obj.hAuxGUIs = [];
            end
            ...
            for i=1:length(obj.guiNames)
                ...
                guiShortName = guiName;
                
                hGUI = feval(guiName,obj.hModel,obj);
                if isa(hGUI, 'most.Gui')
                    inds = strfind(guiName,'.');
                    if ~isempty(inds)
                        guiShortName = guiName(inds(end)+1:end);
                    end
                    
                    obj.hGuiClasses.(guiShortName) = hGUI;
                    hProp = obj.addprop(['h' guiShortName]);
                    obj.(['h' guiShortName]) = hGUI;
                ...
                obj.hGUIData.(guiShortName) = handles;
            end
            ...
            for i=1:length(propNames)
                csv = most.types.struct.name2csv(propNames{i});
                ...
                if isscalar(csv)
                    ...
                    propObjName = most.types.struct.csv2name(csv{1:end-1});
                    fullPropName = [propObjName '.' pname];
                    try
                        hMdl = eval(['obj.hModel.' propObjName]);
                    catch
                        most.idioms.dispError(['Error: Controller initialization failed to find property ''' fullPropName '''.\n']);
                        succ = false;
                        continue;
                    end
                end
        ...
        function reprocessSubMdlPropBindings(obj, SubMdlName)
            propNames = obj.propNames;
            
            for i=1:length(propNames)
                fullPropName = propNames{i};
                if any(strfind(fullPropName,[SubMdlName '.']) == 1)
                    csv = most.types.struct.name2csv(fullPropName);
                    pname = csv{end};
                    propObjName = most.types.struct.csv2name(csv{1:end-1});
                    hMdl = eval(['obj.hModel.' propObjName]);
                    fullPropName_ = zzlclDot2Score(fullPropName);
                    ...
                    if isfield(pbinding,'GuiIDs')
                        %recreate listener
                        delete(obj.hGUIBindingPostSetListeners.(fullPropName_));
                        obj.hGUIBindingPostSetListeners.(fullPropName_) = most.ErrorHandler.addCatchingListener(hMdl,pname,'PostSet',@(src,evnt)obj.updateView(fullPropName));
                        obj.updateView(fullPropName);
                    end
                    
                    if isfield(pbinding,'Callback')
                        %recreate listener
                        delete(obj.hCallbackPostSetListeners.(fullPropName_));
                        obj.hCallbackPostSetListeners.(fullPropName_) = most.ErrorHandler.addCatchingListener(hMdl,pname,'PostSet',@(src,evnt)obj.dispatchCallback(src,evnt,pbinding.Callback));
                        feval(pbinding.Callback,obj,[],[]);
                    end
                end
            end
        end
        ...
        function initialize(obj)
            ...
            try
                for i=1:numel(pnames)
                    ...
                    csv = most.types.struct.name2csv(pnamefull);
                    pname = csv{end};
                ...
                most.idioms.dispError('Error occurred in initializing property ''%s'' belonging to root object  of class ''%s''\n',pnames{i},class(obj.hModel));
                ME.rethrow();
            end
        end
        
        function delete(obj)
            % delete  deletes the controller and closes all associated guis
            ...
            % gets bound.
            %fprintf('most.Controller::delete()\n');
            
            %Delete programmatic gui classes
            gc = fieldnames(obj.hGuiClasses);
            for i = 1:numel(gc)
                most.idioms.safeDeleteObj(obj.hGuiClasses.(gc{i}));
            end
            
            %Delete figure handles explicitly, because HandleVisiblity='callback'
            tf = ishandle(obj.hGUIsArray);
            most.idioms.safeDeleteObj(obj.hGUIsArray(tf));          
            obj.hGUIsArray=[];
        ...
        function [pcontrolTags2Data, pcontrolTags2Handles] = zzzProcessPropBindings(obj,fullPropName)         
            ...
            if isfield(pbinding,'GuiIDs')
                if ~isempty(hMdl)
                    ...
                    obj.hGUIBindingPostSetListeners.(fullPropName_) = [];
                end
            ...
            if isfield(pbinding,'Callback')
                ...
                if ~isempty(hMdl)
                    obj.hCallbackPostSetListeners.(fullPropName_) = most.ErrorHandler.addCatchingListener(hMdl,pname,'PostSet',@(src,evnt)obj.dispatchCallback(src,evnt,callbackName));
                else
                    obj.hCallbackPostSetListeners.(fullPropName_) = [];
                end
    ...
    methods
        ...
        function zzzProcessPropFields(obj,fnames,varargin)
            for i=1:length(fnames)
                ...
                if ~isempty(intersect(fieldnames(fieldVal),{'GuiIDs' 'Callback'}))
                    %                         if nargin > 1
                    %                             %propNamesCache{end+1} = [most.types.struct.csv2name(varargin) '.' fieldVal]; %#ok<AGROW>
                    %                             propNamesCache{end+1} = fullFieldNameCell
                    %                         else
                    %                             propNamesCache{end+1} = fieldVal; %#ok<AGROW>
                    %                         end
                    obj.propNamesCache{end+1} = most.types.struct.csv2name(fullPropNameCell{:});
                else
        ...
        function set.robotMode(obj,val)
            ...
            val = logical(val);
            oldval = obj.robotMode;
            
            if val ~= oldval
                postSetListeners = struct2cell(obj.hGUIBindingPostSetListeners);
                postSetListeners = [postSetListeners{:}];
                [postSetListeners.Enabled] = deal(~val);
                
                callbackListeners = struct2cell(obj.hCallbackPostSetListeners);
                callbackListeners = [callbackListeners{:}];
                [callbackListeners.Enabled] = deal(~val);
            end
    ...
    methods     
        function raiseAllGUIs(obj)
            % raiseAllGUIs   raises all visible guis to top
            %
            %   obj.raisAllGUIs()  returns nothing
            
            % maintain stacking order of windows
            allFigs = allchild(0);
            mask = ismember(allFigs,obj.hManagedGUIs);
            hFigs = allFigs(mask);
            hFigs = flipud(hFigs);
            
            arrayfun(@(x)obj.raiseGUI(x),hFigs);
        end
        
        function showAllGUIs(obj)
            % showAllGUIs    make all guis handled by this controller visible
            %
            %   obj.showAllGUIs()   returns nothing
        ...
        function raiseGUI(obj,guiH) %#ok<MANU>
            % raiseGUI   make gui visible and raise to top
            %
            %    obj.raiseGUI(guiH)   raises gui given by 'guiH'. guiH is either  the name of the gui or a gui handle
            
            if most.idioms.isstring(guiH)
                guiH = obj.hGUIs.(guiH);
            end
        ...
        function showGUI(obj,src,~)
            ...
            %
            %    obj.showGUI(guiH)   shows gui given by 'guiH'. guiH is either  the name of the gui or a gui handle
        ...
        function hideGUI(obj,src,~)
            % hideGUI   make gui invisible
            %
            %    obj.hideGUI(guiH)   shows gui given by 'guiH'. guiH is either the name of the gui or a gui handle
        ...
        function updateView(obj,src,~,fullPropName)
            % updateView   refreshes all gui elements
            %
            %   obj.updateView()  returns nothing
            ...
            if nargin < 2
                ...
                props = fieldnames(obj.prop2BindingsStruct);
                src = cellfun(@(prop)zzlclScore2Dot(prop),props,'UniformOutput',false);
            end
        ...
        function executeCallbacks(obj)
            % executeCallbacks   execute all callbacks to update gui
            %
            %   obj.executeCallbacks()  returns nothing
            
            if obj.robotMode
                return
            end
            
            fns = fieldnames(obj.hCallbackPostSetListeners);
            for idx = 1:length(fns)
                lh = obj.hCallbackPostSetListeners.(fns{idx});
                cellfun(@(src)lh.Callback(src,[]),lh.Source);
            end
        end
        
        function updatePropControlView(obj,pCtl)
            % updatePropControlView updates view for all properties associated with specified PropControl(s)
            %
            %   obj.updatePropControlView(pCtl)  update view for control defined by 'pCtl', where pCtl is either the handle to the PropControl or a name of a PropControl
            if ischar(pCtl)
    ...
    methods (Hidden)
                ...
                switch ctlStyle
                    ...
                    case {'edit' 'listbox'}
                        if zlclIsPropNumericClass(hMdl.mdlPropAttributes, pname)
                            PropertyAttributes = hMdl.mdlPropAttributes.(pname);
                            ...
                            hasEmptyProps = any(isempty(propVal));
                            isEmptyValueAllowed = isfield(PropertyAttributes, 'AllowEmpty')...
                                && PropertyAttributes.AllowEmpty;
                            if hasEmptyProps && ~isEmptyValueAllowed
                                status = 'revert';
                            end
                        end
                end
        ...
        function updateModelErrorFcn(obj,ME)
            ...
            switch ME.identifier
                case 'most:Model:invalidPropVal'
                    most.idioms.dispError('WARNING: Attempted to set invalid property value via GUI control. Attempt ignored. \n');
        ...
        function attachPropBindingsToToolTipStrings(obj,prefix)
            if nargin < 2 || isempty(prefix)
                prefix = '';
            end
            
            props = fieldnames(obj.prop2ControlStruct);
            for idxprop = 1:length(props)
                prop = props{idxprop};
                bindings = obj.prop2ControlStruct.(prop);
                for idx = 1:length(bindings)
                    try
                        str = get(bindings{idx},'TooltipString');
                        if ~isempty(str)
                            str = [str '\n'];
                        end
                        set(bindings{idx},'TooltipString',sprintf([str '%s%s'],prefix,zzlclScore2Dot(prop)));
                    catch
                    end
                end
            end            
    ...
    methods
        function s = ctlrCurrentGUILayout(obj)
            % ctlrCurrentGUILayout   Get layout state (positions, visibility, advanced-panel-ness,
            % etc) for all managed GUIs.
            %
            % s: struct. fields: GUI tags. values: struct with fields
            % 'Position', 'Visible', 'Toggle'.
            %
            %    s = obj.ctlrCurrentGUILayout()
        ...
        function ctlrLoadGUILayoutFromStruct(obj,layoutStruct)
            % ctlrLoadGUILayoutFromStruct   loads layout state (positions, visibility, advanced-panel-ness,
            %           etc) for all managed GUIs from layoutStruct
            %
            %    obj.ctlrCurrentGUILayout(ctlrLoadGUILayoutFromStruct)   returns nothing
        ...
        function ctlrSaveGUILayout(obj,filename)
            % ctlrSaveGUILayout   takes snapshot of layout state for all managed GUIs and save to
            % filename. If filename is not supplied, uigetfile is called.
            %
            %    obj.ctrlSaveGUILayout            
        ...
        function ctlrLoadGUILayout(obj,filename)
            % ctlrLoadGUILayout   loads layout state from file. If filename is not supplied,
            %    uigetfile is called.
            %
            %    obj.ctlrLoadGUILayout(filename)
        ...
        function h = figure(obj,tag,varargin)
            % figure  if tag matches the Tag of a managed figure, that figure is
            ...
            %         passed to figure().
            %
            %   obj.figure(tag)   % returns the figure with name 'tag' or, if not found creates a new figure
            %   obj.figure(tag,'Prop1','Val1,...,'PropN','ValN') % returns the figure with name 'tag' or, if not found creates a new figure with defined properties and values
        ...
        function registerGUI(obj,figH)
            % registerGUI   Register a figure to be managed. Lay the figure out if layout
            %               info is available.
            %
            %    obj.registerGUI(figH)   figH is the handle of the figure  to be registered
            
            assert(ishandle(figH));
            assert(~isempty(figH.Tag),'Figure must have a tag to be registered.');
            guis = obj.hManagedGUIs;
            tags = get(guis,{'Tag'});
            tag = get(figH,'Tag');
        ...
        function unregisterGUI(obj,figH)
            % unregisterGUI    unregister gui so that is no longer managed
            %                  by controller
            %
            %    obj.unregisterGUI(figH)   figH is the handle of the figure  to be unregistered
        ...
        function layoutGUI(obj,figH)
            ...
            if isfield(obj.guiLayoutInfo,tag)
                ...
                if ~isempty(toggleState)
                    if ~most.gui.AdvancedPanelToggler.isFigToggleable(figH)
                        return
                    end
                ...
                elseif strcmp(get(figH, 'Resize'), 'off')
                    pos = get(figH,'Position');
                    pos(1:2) = layoutInfo.Position(1:2);
                    set(figH,'Position',pos);
                else
    ...
    methods (Access=protected)
        ...
        function dispatchCallback(obj,src,~,callbackName)
             if obj.robotMode
                 return
             end
             feval(callbackName,obj,src,[]);
        end
...
%--------------------------------------------------------------------------%
% Controller.m                                                             %
% Copyright © 2025 Vidrio Technologies, LLC                                %
%                                                                          %
