classdef MachineDataFile < handle
    ...
    methods
        ...
        function tfNewVar = writeVarToHeading(obj,hding,varName,val,custcmt,commitNow)
            assert(obj.isLoaded);
        ...
        function renameSection(obj,oldHdg,newHdg)
            idx = strcmp({obj.fHData.heading},oldHdg);
            obj.fHData(idx).rows{1} = ['%% ' newHdg];
            most.MachineDataFile.write(obj.fMDFName,obj.fHData);
        end
